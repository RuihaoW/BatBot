...
def butter_bandpass(lowcut, highcut, fs, order = 512):
	nyq = 0.5*fs
	low = lowcut/nyq
	high = highcut/nyq
...
def butter_bandpass_filter(data, lowcut, highcut, fs, order = 512):
	...
	while file_counter < iterations:
		...
		try:
			...
			if maketimevec == True: #Runs once
				...
				#Filter here
				lp_current_filtered = butter_bandpass_filter(lp_current_array, lowcut, highcut, fs, order = 6)
				rp_current_filtered = butter_bandpass_filter(rp_current_array, lowcut, highcut, fs, order = 6)
